...
<style>
/* container: fluid with a desktop cap */
.carousel2{
  position:relative;
  width:100%;
...
.carousel2 .slides{
  ...
  overflow:hidden;
  border-radius:8px;
  aspect-ratio:16/9;    /* default shape (auto height) */
  background:#f2f2f2;
}
...
.carousel2 .slide{
  ...
  object-fit:cover;      /* use `contain` if you prefer full image */
...
.carousel2 .arrows .prev{ left:8px }
.carousel2 .arrows .next{ right:8px }
